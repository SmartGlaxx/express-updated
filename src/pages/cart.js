...
@media screen and (max-width: 767px){
    ...
    .cart-purchase{margin: 0rem;margin-top:4rem;width: 100%}
}
...
const Cart =()=>{
    ...
    const cartTotal = cartItems.reduce((total, item)=>{
        ...
        const totalAmount = amount * (price/500)
     ...
     const shippingCalc = cartItems.reduce((shippingTotal, item)=>{
         ...
         if(shipping == 'false'){
            return shippingTotal += (((price/500) * 5)/100)
          }else{
                ...
                <div className='cart'>
                    ...
                    className={`${selectColor == 'white' ? 'itemColor darken' : selectColor == 'black' ? 'itemColor whiten' : 'itemColor' }`}>
                                ...
                                <div className='cart-desc'>
                                    ...
                                    <div><strong>  Unit Price: </strong>C${NumberFormat(price/500)}</div>
                                    <div>{shipping == 'true' ? <><strong>Shipping :</strong> Free </>: <><strong>Shipping : </strong> C${NumberFormat((price/500) * (5/100))}</> }</div>
                                    {shipping == 'true' ? 
                                    <div><strong>  Item Total Cost 2: </strong>C${ NumberFormat((amount * (price/500)))} </div> :
                                    <div><strong>  Item Total Cost 1: </strong>C${ NumberFormat((amount * (price/500)) + ((price/500) * 5)/100)} </div> 
                                     }
                         ...
                         {cartItems.length > 0 ?  <div className='cart-totals'> 
                            <div>
                            <div>Subtotal :  <span className='amount'>C${NumberFormat(cartTotal)}</span></div>
                            <div>Shipping :  <span className='amount'>C${NumberFormat(shippingCalc)}</span></div>
                            <hr />
                            <div className='total'><span>Total :</span> <span className='amount'>C${NumberFormat(cartTotal + shippingCalc)}</span></div>
                            </div>
                           {   isAuthenticated ?  <div>
